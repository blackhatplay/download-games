...
function updateCategory() {
  ...
  xhr.open("GET", "https://testtravelhorse.000webhostapp.com/api/getcatgip.php", true);
...
function getLink(link) {
  var xhr = new XMLHttpRequest();
  xhr.open("POST", "https://testtravelhorse.000webhostapp.com/api/apigip.php", true);
            ...
            for (i = 0; i < res.length; i++) {
              if (i == 0) {
                document.getElementById(
                  "download"
                ).innerHTML += `<a href="${res[i]}" target="_blank">Download</a>`;
              } else {
                document.getElementById(
                  "download"
                ).innerHTML += `<a href="${res[i]}" target="_blank">Download Mirror</a>`;
              }
            }
          } else {
            document.getElementById(
...
function getdata(type, value, pageNo = 1) {
  ...
  if (!value == "") {
    ...
    fetch(`https://testtravelhorse.000webhostapp.com/api/getpostgip.php?${temp}`)
      .then((res) => res.json())
...
searchList.addEventListener("click", (e) => {
  if (e.target.classList.contains("get-event")) {
    ...
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "https://testtravelhorse.000webhostapp.com/api/getpostgip.php", true);
